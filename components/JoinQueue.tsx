...
function JoinQueue({
  ...
  const handleJoinQueue = async () => {
    try {
      ...
      if (
        ...
        error.message.includes("already in waiting list")
      ) {
  ...
  return <div>
    {(!queuePosition || queuePosition.status === WAITING_LIST_STATUS.EXPIRED || 
    (queuePosition.status === WAITING_LIST_STATUS.OFFERED && queuePosition.offerExpiresAt && queuePosition.offerExpiresAt < Date.now())) && (
       <>
       {isEventOwner ? (
